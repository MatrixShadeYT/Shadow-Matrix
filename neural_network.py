...
class Layer_Dense:
    ...
    def forward(self,inputs):
        ...
        self.SM = numpy.exp(self.output-np.max(self.output,axis=1,keepdims=True)) / numpy.sum(numpy.exp(self.output-np.max(self.output,axis=1,keepdims=True)),axis=1,keepdims=True)
